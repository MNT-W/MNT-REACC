...
# on message
        ...
        if message.content[6:] == 'processes':
            ...
            for proc in range(1, len(processes_list)):
                if len(processes) < 1800:
                    ...
                    #.log List of running processes is below 1800 characters. MNT won\'t divide it 
                else:
                    #.log List of running processes is above 1800 characters. MNT will divide it into smaller pieces 
                    processes += '\n**' + str(proc) + ') **' + str(processes_list[proc])
    ...
    if message.content.strip() == '.kill':
        ...
        embed.set_author(name="MNT-REACC", icon_url="hhttps://raw.githubusercontent.com/MNT-W/MNT-REACC/py-dev/resources/icons/embed_icon.png")
        reaction_msg = await message.channel.send(embed=embed); await reaction_msg.add_reaction('🔴')
