...
# Github: MNT-W/MNT-REACC
# Author: Montana
# Description: Bash script to compile MNT-REACC under Linux with wine
...
if [ "$mode" == 'c' ]; then
    ...
    if [ "$create_venv" == 'y' ]; then
        if [ "$package_manager" == '4' ]; then
            ...
            wine64 python -m venv MNT
        else
            wine python -m pip install wheel setuptools
            wine python -m venv MNT
        fi
    ...
    if [ "$package_manager" == '4' ]; then
        wine64 call ".\\MNT\\Scripts\\activate.bat"
    else
        wine call ".\\MNT\\Scripts\\activate.bat"
    fi
...
echo -e "\e[33m# https://github.com/MNT-W/MNTREACC             #\e[0m"
echo -e "\e[33m#===============================================#\e[0m"
echo
echo
